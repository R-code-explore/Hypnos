...
&& empty($_POST["dispo"])){
    ...
    if(!array_key_exists($extension_first_image, $allowed) || !in_array($first_image_type, $allowed)
    || !array_key_exists($extension_img1, $allowed) || !in_array($img1_type, $allowed)
    || !array_key_exists($extension_img2, $allowed) || !in_array($img2_type, $allowed)
    || !array_key_exists($extension_img3, $allowed) || !in_array($img3_type, $allowed)){
    ...
    $sql = "UPDATE `suites` SET `first_image`='$first_image',`description`='$description',`prix`='$prix',`glr_image1`='$img1',`glr_image2`='$img2',`glr_image3`='$img3',`dispo`='$dispo' WHERE `id` = '$id'";
